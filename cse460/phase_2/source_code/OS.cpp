...
void OS::run() {
    ...
    //Computes System Accounting
    double final = (sys_time / 1000.0);
    
    double thru = final / program_count;
    
    double util1 = (sys_time - idle_time);
    double util2 = (util1 / sys_time);
    int util = (util2 * 100);
    
    //output all accounting
    printf ("%20s%10d%s%f%s\n", "System time: ", sys_time, " ticks, ", final, " seconds");
    printf ("%20s%10f\n", "Program Count: ", program_count);
    printf ("%20s%10d%s\n", "Idle Time: ", idle_time, " ticks");
    printf ("%20s%9d%s\n", "System Utilization: ", util, "%");
    printf ("%20s%10f%s\n", "Throughput: ", thru, " sec/process");
}
...
void OS::output_stats() {
    ...
    ofile << endl << "CPU Time: " << running->cpu_time << endl;
    ofile << "Waiting time: " << running->wait_time << endl;
    ofile << "Turn around time: " << running->turn_around_time << endl;
    ofile << "I/O time: " << running->io_time << endl;
    ofile << "Largest stack size: " << running->largest_stack_size << endl;
